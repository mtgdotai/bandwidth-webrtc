...
function BWCall(data) {
	...
	if (data.authToken){
		callOptions.extraHeaders.push("X-Callsign-Token: " + data.authToken);
	}
	if (data.options && data.options.identity){
		var e164 = Util.parsePhoneNumber(data.options.identity);
		callOptions.extraHeaders.push("P-Preferred-Identity: <sip:" + e164 + "@" + data.domain + ">");
	}
	...
	this.getRemoteStream = function() {
		return data.session.getRemoteStreams()[ 0 ];
	};
